...
function _deprecated_function($fn, $msg)
{
...
function wp_debug_backtrace_summary($ignore_class = null, $skip_frames = 0, $pretty = true)
{
    ...
    foreach ($trace as $call) {
        if ($skip_frames > 0) {
            ...
            } elseif (in_array($call['function'], ['include', 'include_once', 'require', 'require_once'])) {
                $caller[] = $call['function'] . "('" . str_replace([WP_CONTENT_DIR, ABSPATH], '',
                        $call['args'][0]) . "')";
            } else {
...
class wpdb
{
     ...
     * @param int $blog_id Optional. The blog_id to prefix.
     *    Defaults to wpdb::$blogid. Used only when prefix is requested.
    ...
    public function prepare($query, $args)
    {
        ...
        if (strpos($query, '%') === false) {
            _doing_it_wrong('wpdb::prepare',
                sprintf(__('The query argument of %s must have a placeholder.'), 'wpdb::prepare()'), '3.9');
        }
...
"), htmlspecialchars($this->dbhost, ENT_QUOTES)), 'db_connect_fail');
    ...
    function _insert_replace_helper($table, $data, $format = null, $type = 'INSERT')
    {
        ...
        $sql = "{$type} INTO `$table` (`" . implode('`,`', $fields) . "`) VALUES (" . implode(",",
                $formatted_fields) . ")";
