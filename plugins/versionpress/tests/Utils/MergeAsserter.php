...
class MergeAsserter {
    ...
    public static function assertMergeConflict($mergeCommand) {
        ...
        if ($exitCode == 0) {
            Assert::fail('Expected merge conflict, got clean merge instead');
        } else {
            if ($exitCode != 1) {
                Assert::fail('Merge ended with an unexpected exit code ' . $exitCode);
            }
        }
    }
    ...
    public static function assertCleanMerge($mergeCommand) {
        ...
        if ($exitCode == 1) {
            Assert::fail('Merge ended with a conflict, clean merge was expected');
        } else {
            if ($exitCode != 0) {
                Assert::fail('Merge ended with an unexpected exit code ' . $exitCode);
            }
